defmodule SoonReady.UserAuthentication.Researcher do
  ...
  def execute(_aggregate_state, %RegisterResearcher{} = command) do
    with {:ok, %{id: user_id} = user} <- SoonReady.UserAuthentication.UserAccount.register_user_with_password(command.username, command.password, command.password_confirmation) do
